...
.controls{display:flex;gap:12px;align-items:center;margin-bottom:24px;flex-wrap:wrap;justify-content:center}
input[type="date"]{padding:10px 16px;border-radius:8px;border:1px solid #334155;background:#1e293b;color:#e2e8f0;font-size:16px;outline:none}
input[type="date"]:focus{border-color:#f59e0b}
button{padding:10px 24px;border-radius:8px;border:none;font-size:16px;cursor:pointer;transition:all .2s;font-weight:500}
.btn-primary{background:linear-gradient(135deg,#f59e0b,#ef4444);color:#fff}
.btn-primary:hover{transform:translateY(-1px);box-shadow:0 4px 12px rgba(245,158,11,.4)}
.btn-primary:disabled{opacity:.5;cursor:not-allowed;transform:none;box-shadow:none}
.btn-download{background:#10b981;color:#fff;display:none}
.btn-download:hover{background:#059669}
.status{color:#94a3b8;margin-bottom:16px;font-size:14px;min-height:20px;text-align:center}
.status.error{color:#ef4444}
.result-area{max-width:960px;width:100%;display:flex;flex-direction:column;align-items:center;gap:16px}
video{width:100%;max-width:960px;border-radius:12px;background:#000;display:none}
canvas{display:none}
.screenshot-preview{display:none;width:100%;max-width:960px;border-radius:12px;border:2px solid #334155}
.seek-controls{display:none;gap:8px;align-items:center;flex-wrap:wrap;justify-content:center}
.seek-controls button{padding:6px 14px;font-size:13px;background:#1e293b;color:#e2e8f0;border:1px solid #334155}
...
<script>
const datePicker=document.getElementById('datePicker'),fetchBtn=document.getElementById('fetchBtn'),captureBtn=document.getElementById('captureBtn'),downloadBtn=document.getElementById('downloadBtn'),statusEl=document.getElementById('status'),player=document.getElementById('player'),canvas=document.getElementById('canvas'),preview=document.getElementById('preview'),seekControls=document.getElementById('seekControls'),timeDisplay=document.getElementById('timeDisplay');
let hls=null,currentTitle='';
const y=new Date();y.setDate(y.getDate()-1);datePicker.value=y.toISOString().split('T')[0];datePicker.max=new Date().toISOString().split('T')[0];
player.addEventListener('timeupdate',()=>{timeDisplay.textContent=player.currentTime.toFixed(1)+'s'});
async function fetchVideo(){
  const dateStr=datePicker.value.replace(/-/g,'');
  if(!dateStr){setStatus('请选择日期',true);return}
  fetchBtn.disabled=true;setStatus('正在获取视频信息...');
  captureBtn.style.display='none';downloadBtn.style.display='none';preview.style.display='none';seekControls.style.display='none';
  try{
...
function seekTo(t){player.currentTime=t}
function captureFrame(){
  canvas.width=player.videoWidth;canvas.height=player.videoHeight;
  canvas.getContext('2d').drawImage(player,0,0);
  const u=canvas.toDataURL('image/png');preview.src=u;preview.style.display='block';
  downloadBtn.style.display='inline-block';setStatus('截图已生成！点击"下载截图"保存到本地')}
function downloadImage(){
  const a=document.createElement('a');a.href=preview.src;
  a.download=currentTitle.replace(/[^\\w\\u4e00-\\u9fff]/g,'_')+'_封面.png';a.click()}
function setStatus(m,e=false){statusEl.textContent=m;statusEl.className='status'+(e?' error':'')}
</script>
</body>
</html>`;
...
export default {
  async fetch(request) {
    ...
    if (url.pathname === '/api/video-info') return handleVideoInfo(url);
...
async function handleVideoInfo(url) {
  ...
  try {
    const dayResp = await fetch(`https://tv.cctv.com/lm/xwlb/day/${date}.shtml`, {
      headers: { 'User-Agent': 'Mozilla/5.0 (Windows NT 10.0; Win64; x64) AppleWebKit/537.36' },
    });
    if (!dayResp.ok) return json({ error: `该日期无数据: ${date}` }, 404);
    const dayHtml = await dayResp.text();

    const videoLinkMatch = dayHtml.match(/href="(https:\/\/tv\.cctv\.com\/\d{4}\/\d{2}\/\d{2}\/VIDE[^"]+\.shtml)"/);
    if (!videoLinkMatch) return json({ error: '未找到该日期的新闻联播视频' }, 404);

    const videoResp = await fetch(videoLinkMatch[1], {
      headers: { 'User-Agent': 'Mozilla/5.0 (Windows NT 10.0; Win64; x64) AppleWebKit/537.36' },
    });
    const videoHtml = await videoResp.text();
    const guidMatch = videoHtml.match(/var\s+guid\s*=\s*"([a-f0-9]+)"/);
    if (!guidMatch) return json({ error: '无法解析视频信息' }, 500);
...
async function handleProxy(url) {
  ...
  let targetFixed = target.startsWith('http://') ? 'https://' + target.slice(7) : target;
  let hostname;
  try { hostname = new URL(targetFixed).hostname; } catch { return new Response('Invalid URL', { status: 400 }); }

  if (!hostname.includes('cntv') && !hostname.includes('cctv') && !hostname.includes('lxdns') && !hostname.includes('cdn20')) {
